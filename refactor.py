...
class Pokemon:
    ...
    def __init__(self, name: str, level: str,
        ...
        self.nature = nature
        self.item = concat(item.split("_"))
        self.ivs  = ivs 
        self.moves = Pokemon.normalize(moves)

    def ivsToString(array: list[str]):
        start = "IVs: " 
        for i, value in enumerate(array):
            if i == 0:
                start += value + " " + statOrder[i]
            else:
                start += " / " + value + " " + statOrder[i]
    ...
    def normalize(array: list[str]):
        for move in movez:
            try:    
                i = array.index(move.replace(" ", "").lower().capitalize())
            except:
                continue
            array[i] = move
...
file = open("battles.txt", "rt")
out = open("battles_formatted.txt", "wt")

trainer = {}
line = file.readline()
while line:
    
    if line.__contains__("static c"):
        if trainer:
            out.write(trainer["name"] + "\n")
            for pk in trainer["pokemons"]:
                out.write(pk.toString() + "\n")
        trainer = {
            "name" : line.split(" ")[4].replace("sExpertBossBattle_", "").replace("[]", ""),
            "pokemons" : []
        }
    elif line.__contains__(".species"):
        name = line.split(" ")[2][8:-2].lower().capitalize()
        nature = file.readline().split(" ")[2][7:-2].lower().capitalize()
        level = file.readline().split(" ")[2][:-2]
        ivs = []
        for i in range(6):
            ivs.append(file.readline().split(" ")[2][:-1])
        line = file.readline()
        while line.__contains__("Ev = "): # 3 pokemons in the docs have Evs for some reason when i'm making this script so i just ignore them
            line = file.readline()
            continue

        ability = line.split("//")[1][8:-1].lower().capitalize()  
        item = file.readline().split(" ")[2][5:].replace(",", "").lower().capitalize()
        file.readline()
        file.readline()
        moves = []
        moves.append(file.readline().replace(" ", "")[8:-2].lower().capitalize())
        moves.append(file.readline().replace(" ", "")[8:-2].lower().capitalize())
        moves.append(file.readline().replace(" ", "")[8:-2].lower().capitalize())
        moves.append(file.readline().replace(" ", "")[8:-2].lower().capitalize())

        trainer["pokemons"].append(Pokemon(name, level, nature, ability, item ,ivs , moves))
        for i in range(6):
            file.readline()
    line = file.readline()
